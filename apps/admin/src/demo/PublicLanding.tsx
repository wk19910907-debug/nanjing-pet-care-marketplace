...
const services: ReadonlyArray<{
  type: ServiceType; icon: string; title: string; copy: string; action: string;
}> = [
  { type: 'CAT_FEEDING', icon: '猫', title: '上门喂猫', copy: '喂食换水、猫砂清理、宠物状态反馈。', action: '预约喂猫' },
  { type: 'DOG_WALKING', icon: '犬', title: '上门遛狗', copy: '牵引散步、饮水照看、服务状态反馈。', action: '预约遛狗' },
];
...
export function PublicLanding({ catalog, children, onStartOrder, onQuoteStartOrder, quoteSelection, onQuoteChange }: PublicLandingProps) {
  ...
  return <>
    <nav className="public-nav" aria-label="官网导航">
      ...
      <div><a href="#services">服务介绍</a><a href="#safeguards">服务保障</a><a href="#faq">常见问题</a><button type="button" disabled={!bookingAvailable} onClick={onStartOrder}>立即预约</button></div>
    </nav>
    ...
    <section id="services" className="landing-section public-services" aria-labelledby="services-title">
      ...
      <div className="price-grid">
        {availableServices.map((service) => <article className="price-card" key={service.type}><span className="service-icon" aria-hidden="true">{service.icon}</span><div><h3>{service.title}</h3><p>{service.copy}</p></div><strong>{startingPrice(catalog!.services[service.type].basePriceFen)}</strong><button type="button" onClick={() => { onQuoteChange({ ...quoteSelection, serviceType: service.type }); onQuoteStartOrder(); }}>{service.action}</button></article>)}
        {!bookingAvailable && <p className="pilot-empty">服务配置暂不可用，请稍后重试。</p>}
      </div>
      {catalog && <PublicQuote catalog={catalog} selection={quoteSelection} onChange={onQuoteChange} onStartOrder={onQuoteStartOrder}/>}
    </section>
