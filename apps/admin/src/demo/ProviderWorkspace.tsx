...
export function ProviderWorkspace(props: { state: DemoState; providerId: string; submitApplication(draft: ProviderApplicationDraft): boolean; start(orderId: string): void; report(orderId: string, report: ServiceReport): void }) {
  ...
  return <section className="workspace"><ProviderApplicationPanel applications={props.state.providerApplications} submit={props.submitApplication} /><div className="section-title"><div><span className="eyebrow">PROVIDER</span><h2>今日服务任务</h2></div><p>当前身份：王小宁 · 已认证</p></div>
