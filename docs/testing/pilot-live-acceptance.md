...
4. 构建 pilot UI 并启动真实 Fastify pilot server。三个主身份均通过页面上的本地角色入口创建会话；运行器不 bootstrap 管理员邀请，也不通过 Playwright 环境变量或控制端点传递邀请码。
5. 启动一个 Playwright worker，创建恰好三个隔离浏览器上下文：`ADMIN`、`OWNER` 和 `PROVIDER`。仅在同角色 IDOR 隔离需要独立身份时，测试才用隐藏兼容 API 在内存中创建并兑换第二个宠主和服务人员邀请；二者顺序复用后两个上下文，不创建第四个上下文，也不显示或记录邀请码。
...
运行器不会打印数据库密码、加密密钥、认证 pepper、会话值或兼容 API 产生的一次性邀请码。不要使用 `DEBUG=*`、网络录制或永久 trace 重跑含真实试点数据的验收。

## 覆盖的闭环

- 主 `ADMIN`、`OWNER` 和 `PROVIDER` 从可见的本地角色入口进入并设置展示昵称；登录页和管理员工作区均无邀请码登录、创建或管理控件。
...
- 原宠主和服务人员注销后，同一上下文的后续请求及携带已撤销旧 Cookie 的请求均返回 401。第二宠主和第二服务人员仅为同角色 IDOR 验收通过隐藏兼容 API 创建；第二宠主不能读取、确认、报告或用第一宠主的宠物/地址 ID 下单；第一服务人员另有一个仍为 `PENDING` 的派单邀请，第二服务人员对该精确邀请执行接受写入得到 403/404，随后只能接受自己的 `PENDING` 派单邀请，也不能读取第一服务人员订单、地址或证据。
- 管理员、宠主和服务人员的越权 API 请求返回 403；匿名证据读取请求返回 401。
- 中途真实停止并重新启动 API 进程；原三个 Cookie 会话和 PostgreSQL 订单数据继续有效。
- 从角色入口、昵称、申请、宠主表单/订单、审核/费用/派单、履约到最终状态的每个主要阶段，390×844 视口下可见交互控件高度至少 44px 且无横向溢出；三个现有上下文还会在各角色关键中间态和最终工作区切换到 1280×800，执行相同的控件、溢出、存储、Cookie 与敏感字段断言，然后恢复 390×844。
...
所有业务请求都进入实际应用；测试文件不调用 `page.route`，也不替换 Fastify 路由、Prisma 或对象存储适配器。本地直接会话路由只在 `development` 和 `test` 注册，且只接受 loopback 请求；生产环境中该路由不存在。
